...
typedef void (^EPLAdInitCompletion)(BOOL);

@interface EPLAd : NSObject
...
/**
 * Initialize EPL Ads SDK
 * @param memberId for initializing the EPL SDK
...
- (void)initWithMemberID:(NSInteger) memberId
        ...
        completionHandler: (EPLAdInitCompletion _Nullable)completionHandler;
...
/**
 * API to check if the EPLAd is already initialised or not
 * @return true / false based on the check, if the EPLAd is initialised or not
 * */
- (BOOL)isInitialised;
