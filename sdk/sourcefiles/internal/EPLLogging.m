...
void _ANLog(EPLLogLevel level, NSString *levelString, char const *logContext, NSString *format, ...) {
    if ([EPLLogManager getANLogLevel] <= level) {
        format = EPLErrorString(format); // returns the format string if error string not found
...
{
    EPLPostNotifications(kANLoggingNotification, nil,
