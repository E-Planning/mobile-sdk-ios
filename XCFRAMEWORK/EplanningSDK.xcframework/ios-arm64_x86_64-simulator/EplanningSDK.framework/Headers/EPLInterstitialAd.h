...
@protocol EPLInterstitialAdDelegate;
...
/**
 ...
 @code
       ...
       self.inter = [[EPLInterstitialAd alloc] initWithPlacementId:@"1326299"];

       // We set ourselves as the delegate so we can respond to the `adDidReceiveAd:' message of the
       // `EPLInterstitialAdDelegate' protocol.  (See the bottom of this file for an example.)
       self.inter.delegate = self;

       // When the user clicks, use the following to open the default browser on the device.
       self.inter.clickThroughAction = EPLClickThroughActionOpenDeviceBrowser;
 ...
 */
@interface EPLInterstitialAd : EPLAdView

@property (nonatomic, readwrite, weak, nullable) id<EPLInterstitialAdDelegate> delegate;
...
/**
 ...
 or your ad network.
 @param placementId the placement ID given from EPL
 @returns void
 */
...
#pragma mark EPLInterstitialAdDelegate

@protocol EPLInterstitialAdDelegate <EPLAdDelegate>
...
/**
 ...
 @code
 - (void)adFailedToDisplay:(EPLInterstitialAd *)ad
 {
     NSLog(@"Oh no, the ad failed to display!");
 }
 @endcode
 */
- (void)adFailedToDisplay:(nonnull EPLInterstitialAd *)ad;
@end
