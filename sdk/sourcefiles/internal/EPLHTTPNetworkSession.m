...
@interface EPLHTTPNetworkTaskData : NSObject
...
@implementation EPLHTTPNetworkTaskData
...
@interface EPLHTTPNetworkSession () <NSURLSessionDataDelegate>
...
@property (nonatomic, strong) NSMutableDictionary<NSURLSessionTask *, EPLHTTPNetworkTaskData *> * sessions;
@property (nonatomic, strong) dispatch_queue_t sessionsQueue;

@end

@implementation EPLHTTPNetworkSession
...
- (void)appendSessionData:(EPLHTTPNetworkTaskData *)data forTask:(NSURLSessionTask *)task {
 ...
 */
- (EPLHTTPNetworkTaskData *)sessionDataForTask:(NSURLSessionTask *)task {
    __block EPLHTTPNetworkTaskData * data = nil;
...
/**
 Appends additional data to the @c responseData field of @c EPLHTTPNetworkTaskData in
...
- (void)appendData:(NSData *)data toSessionDataForTask:(NSURLSessionTask *)task {
    ...
    dispatch_barrier_sync(self.sessionsQueue, ^{
        ...
        EPLHTTPNetworkTaskData * taskData = self.sessions[task];
        if (taskData == nil) {
            return;
        }
...
+ (NSURLSessionTask *)taskWithHttpRequest:(NSURLRequest *)request
                          responseHandler:(void (^ _Nullable)(NSData * data, NSHTTPURLResponse * response))responseHandler
                             errorHandler:(void (^ _Nullable)(NSError * error))errorHandler {
    // Networking task
    NSURLSessionDataTask * task = [EPLHTTPNetworkSession.sharedInstance.adServerSession dataTaskWithRequest:request];

    // Initialize the task data
    EPLHTTPNetworkTaskData * taskData = [[EPLHTTPNetworkTaskData alloc] initWithResponseHandler:responseHandler errorHandler:errorHandler];

    // Update the sessions.
    [EPLHTTPNetworkSession.sharedInstance appendSessionData:taskData forTask:task];
...
+ (void)startTaskWithHttpRequest:(NSURLRequest *)request {
    [EPLHTTPNetworkSession startTaskWithHttpRequest:request responseHandler:nil errorHandler:nil];
}
...
+ (void)startTaskWithHttpRequest:(NSURLRequest *)request
    ...
    NSURLSessionTask * task = [EPLHTTPNetworkSession taskWithHttpRequest:request responseHandler:^(NSData * _Nonnull data, NSHTTPURLResponse * _Nonnull response) {
...
didCompleteWithError:(nullable NSError *)error {
    ...
    EPLHTTPNetworkTaskData * taskData = [self sessionDataForTask:task];
    if (taskData == nil) {
        return;
    }
    ...
    if (httpResponse == nil) {
        NSError *responseError = EPLError(@"Network response is not of type NSHTTPURLResponse", EPLAdResponseCode.NETWORK_ERROR.code);
        EPLLogError(@"%@", responseError);
    ...
    if (httpResponse.statusCode >= 400) {
        NSError * responseError = EPLError(@"connection_failed", EPLAdResponseCode.NETWORK_ERROR.code);
        EPLLogError(@"%@", responseError);
        taskData.errorHandler(responseError);
        return;
    }
    ...
    if (taskData.responseData == nil) {
        NSError * noDataError = EPLError(@"The ad response does not contain data", EPLAdResponseCode.NETWORK_ERROR.code);
        EPLLogError(@"%@", noDataError);
