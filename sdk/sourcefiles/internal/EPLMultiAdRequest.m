...
@interface  EPLMultiAdRequest()  <EPLAdDelegate, EPLNativeAdRequestDelegate, EPLRequestTagBuilderCore>

@property (nonatomic, readwrite, weak, nullable)  id<EPLMultiAdRequestDelegate>  delegate;
...
@property (nonatomic, readwrite, strong)  EPLAdFetcherBase    *adFetcher;
@end
...
@implementation  EPLMultiAdRequest
...
- (nullable instancetype)initWithMemberId: (NSInteger)memberID
                                 delegate: (nullable id<EPLMultiAdRequestDelegate>)delegate
                                  adUnits: (nonnull id<EPLAdProtocolFoundationCore>) firstAdUnit, ...
{
    self = [super self];
    if (!self)  { return nil; }
    ...
    //
    id<EPLAdProtocolFoundationCore>   adUnitArgument  = nil;

    if (! [self addAdUnit:(id<EPLAdProtocolFoundationCore>)firstAdUnit])  { return nil; }

    va_list adUnitArgs;
    va_start(adUnitArgs, firstAdUnit);


    adUnitArgument = va_arg(adUnitArgs, id<EPLAdProtocolFoundationCore>);

    while(adUnitArgument != nil) {
          if (! [self addAdUnit:(id<EPLAdProtocolFoundationCore>)adUnitArgument])  { return nil; }
          adUnitArgument = va_arg(adUnitArgs, id<EPLAdProtocolFoundationCore>);
    }
...
- (nullable instancetype)initWithMemberId:(NSInteger)memberID andDelegate:(nullable id<EPLMultiAdRequestDelegate>)delegate
{
    self = [super self];
    if (!self)  { return nil; }
 ...
 */
- (BOOL)setupWithMemberId:(NSInteger)memberId publisherID:(NSInteger)publisherId andDelegate:(nullable id<EPLMultiAdRequestDelegate>)delegate
{
    if (memberId <= 0) {
        EPLLogError(@"memberId MUST BE GREATER THAN zero (0).");
        return  NO;
    }

    if (publisherId < 0) {
        EPLLogError(@"publisherId MUST BE non-negative.");
        return  NO;
    }
    ...
    _adFetcher  = [[EPLAdFetcherBase alloc] initWithMultiAdRequestManager: self];

    __memberId          = memberId;
    __publisherId       = publisherId;
    __age               = @"";
    __gender            = EPLGenderUnknown;
    __location          = nil;
 ...
 */
- (BOOL)addAdUnit:(nonnull id<EPLMultiAdProtocol,EPLAdProtocolFoundationCore>)newAdUnit
{
    if (!newAdUnit) {
        EPLLogError(@"FAILED to Add newAdUnit, newAdUnit can not be nil");
        return NO;
    }
    
    if(![newAdUnit conformsToProtocol:@protocol(EPLAdProtocolFoundationCore)]){
        EPLLogError(@"FAILED to Add newAdUnit, newAdUnit is not supported by MultiAdRequest object");
        return NO;
    }
    ...
    if ([getProperties count] != 4) {
        EPLLogError(@"FAILED to read newAdUnit properties.");
        return  NO;
    }
    ...
    if (getProperties[3] != nullObj)
    {
        if ([self indexOfAdUnitWithUUIDKey:newUUIDKey] != kMARAdUnitIndexNotFound) {
            EPLLogError(@"IGNORING newAdUnit because it is already managed by this MultiAdRequest object.");
        } else {
            EPLLogError(@"REJECTING newAdUnit because it is managed by another MultiAdRequest object.");
        }
 ...
 * Returns: YES on success; otherwise, NO.
 */
- (BOOL)removeAdUnit:(nonnull id<EPLMultiAdProtocol,EPLAdProtocolFoundationCore>)adUnit
{
    if (!adUnit) {
        EPLLogError(@"FAILED to remove adUnit, adUnit can not be nil");
        return NO;
    }
    
    if(![adUnit conformsToProtocol:@protocol(EPLAdProtocolFoundationCore)]){
        EPLLogError(@"FAILED to remove adUnit, adUnit is not supported by MultiAdRequest object");
        return NO;
    }
    ...
    if ([getProperties count] != 1) {
        EPLLogError(@"FAILED to read adUnit property.");
        return  NO;
    }
    ...
    {
        EPLLogError(@"AdUnit is not managed by this Multi-Ad Request instance.  (%@)", auUUIDKey);
        return  NO;
    }
...
- (BOOL)load
{
    ...
    if (errorString)
    {
        NSError  *sessionError  = EPLError(@"multi_ad_request_failed %@", EPLAdResponseCode.INVALID_REQUEST.code, errorString);
...
{
    EPLLogError(@"publisherId may only be SET WITH INITIALIZERS.");
}
 ...
 * NB  Passes internal pointer back to calling environment.
 */
- (nullable id<EPLMultiAdProtocol>)internalGetAdUnitByUUID:(nonnull NSString *)uuidKey
{
 ...
 */
- (nullable NSArray<id> *)adUnit: (nonnull id<EPLMultiAdProtocol,EPLAdProtocolFoundationCore>)adUnit
                   getProperties: (nonnull NSArray<NSNumber *> *)getTypes
{
    ...
    for (NSNumber *gt in getTypes)
    {
        ...
        switch (getType)
        {
            ...
            default:
                EPLLogError(@"(internal) UNKNOWN MultiAdPropertyType getType.  (%@).", @(getType));
 ...
 */
- (void)adUnit: (nonnull id<EPLMultiAdProtocol>)adUnit
    setManager: (nullable id)delegate
{
    adUnit.marManager =(EPLMultiAdRequest *)delegate;
   return;
}
...
- (NSInteger)indexOfAdUnitWithUUIDKey:(nonnull NSString *)uuidKey
{
    ...
    for (id<EPLMultiAdProtocol> au in self.adUnits)
    {
        adunitIndex += 1;

        if (!au) {
            EPLLogError(@"ELEMENT MISSING from array of AdUnits.");
            continue;
        }
        ...
        {
            EPLLogDebug(@"MATCHED uuidKey.  (%@)", uuidKey);   //DEBUG
            return  adunitIndex;
        }
    }
...
- (void)adRequest:(nonnull EPLNativeAdRequest *)request didFailToLoadWithError:(nonnull NSError *)error
{
    //EMPTY
}
