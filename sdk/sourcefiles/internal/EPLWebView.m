...
#define EPL_USER_DENIED_LOCATION_PERMISSION 1
...
@implementation EPLWebView
    ...
    -(instancetype) initWithSize:(CGSize)size URL:(NSURL *)URL baseURL:(NSURL *)baseURL
    {
        ...
        [[[NSURLSession sharedSession] dataTaskWithRequest: EPLBasicRequestWithURL(URL)
                                         completionHandler: ^(NSData * _Nullable data, NSURLResponse * _Nullable response, NSError * _Nullable error)
          {
              __strong WKWebView  *strongWebView  = weakWebView;
              if (!strongWebView)  {
                  EPLLogError(@"COULD NOT ACQUIRE strongWebView.");
                  return;
              }
    ...
    + (EPLWebView *) fetchWebView {
        EPLWebView *removedWebView;
        
        if(webViewQueue.count > 0){
            removedWebView = [webViewQueue lastObject];
            [webViewQueue removeLastObject];
        } else {
            removedWebView = [[EPLWebView alloc] initWithSize:CGSizeZero];
        }
    ...
    + (void) prepareWebView {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            EPLWebView *webView = [[EPLWebView alloc] initWithSize:CGSizeZero];
    ...
    +(WKWebViewConfiguration *)prepareWebConfiguration {
        ...
        if(!EPLSDKSettings.sharedInstance.locationEnabledForCreative){
            ...
            NSString *execCurrentPositionDenied =  [NSString stringWithFormat:@"navigator.geolocation.getCurrentPosition = function(success, error){ error({ error: { code: %d } });};",EPL_USER_DENIED_LOCATION_PERMISSION];;
...
+ (void) loadWebViewConfigurations {
    ...
    if(omidScript == nil){
        omidScript = [[WKUserScript alloc] initWithSource: [[EPLOMIDImplementation sharedInstance] getOMIDJS]
           injectionTime: WKUserScriptInjectionTimeAtDocumentStart
        forMainFrameOnly: YES];
    }
...
+ (NSString *)mraidJS
{
    NSString *mraidPath = EPLMRAIDBundlePath();
    if (!mraidPath) {
        return @"";
    }
...
+ (NSString *)anjamJS
{
    NSString *sdkjsPath = EPLPathForANResource(@"sdkjs", @"js");
    NSString *anjamPath = EPLPathForANResource(@"anjam", @"js");
