...
@interface EPLLocation ()

@property (nonatomic, readwrite, assign) NSInteger precision;

@end

@implementation EPLLocation

+ (nullable EPLLocation *)getLocationWithLatitude:(CGFloat)latitude
                                       longitude:(CGFloat)longitude
                                       timestamp:(nullable NSDate *)timestamp
                              horizontalAccuracy:(CGFloat)horizontalAccuracy {
    return [EPLLocation getLocationWithLatitude:latitude
                                     longitude:longitude
                                     timestamp:timestamp
...
+ (nullable EPLLocation *)getLocationWithLatitude:(CGFloat)latitude
    ...
    if (invalidPrecision) {
        EPLLogWarn(@"Invalid precision passed in (%d) with location, no rounding will occur", (int)precision);
    }
    ...
    EPLLocation *location = [[EPLLocation alloc] init];
    if (precision <= -1) {
