...
@interface EPLOpenInExternalBrowserActivity()

@property (nonatomic, readwrite, strong) NSURL *URLToOpen;

@end

@implementation EPLOpenInExternalBrowserActivity
...
- (UIImage *)activityImage {
    NSString *iconPath = EPLPathForANResource(@"compass", @"png");
    if (!iconPath) {
        EPLLogError(@"Could not find compass image for 'Open in Safari' sharing option");
        return nil;
    }
...
- (void)performActivity {
    [EPLGlobal openURL:[self.URLToOpen absoluteString]];
