...
@interface EPLMRAIDResizeViewManager () <EPLMRAIDResizeViewDelegate>
...
@property (nonatomic, readwrite, strong) EPLMRAIDResizeProperties *lastResizeProperties;

@property (nonatomic, readwrite, strong) EPLMRAIDResizeView *resizeView;
...
@implementation EPLMRAIDResizeViewManager

#pragma mark - MRAID Resize Validation

+ (BOOL)validateMRAIDResizeFromView:(UIView *)view
               withResizeProperties:(EPLMRAIDResizeProperties *)properties
...
#pragma mark - EPLResizeViewManager Implementation

- (instancetype)initWithContentView:(UIView *)contentView
                         anchorView:(UIView *)anchorView {
    if (contentView == anchorView) {
        EPLLogError(@"%@ Anchor view cannot be the same as the content view", NSStringFromClass([self class]));
        return nil;
    }
    if (!contentView || !anchorView) {
        EPLLogError(@"%@ Content view, anchor view, and root view have to be defined", NSStringFromClass([self class]));
        return nil;
    }
    ...
    if (self = [super init]) {
        ...
        _resizeView = [[EPLMRAIDResizeView alloc] init];
...
- (BOOL)attemptResizeWithResizeProperties:(EPLMRAIDResizeProperties *)properties
    ...
    BOOL allowResize = [EPLMRAIDResizeViewManager validateMRAIDResizeFromView:self.anchorView
...
- (void)resizeWithResizeProperties:(EPLMRAIDResizeProperties *)properties {
...
#pragma mark - EPLResizeViewManagerDelegate

- (void)closeRegionSelectedOnResizeView:(EPLMRAIDResizeView *)resizeView {
