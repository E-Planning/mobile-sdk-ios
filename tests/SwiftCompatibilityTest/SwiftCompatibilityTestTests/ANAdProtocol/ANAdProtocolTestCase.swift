...
class ANAdProtocolTestCase: XCTestCase, ANBannerAdViewDelegate {
    ...
    func test_TC3_LocationInBanner() {
        self.banner = ANBannerAdView.init(frame: CGRect(x: 0, y: 0, width: 300, height: 250), memberId: memberID, inventoryCode: inventoryCode, adSize: CGSize(width: 300, height: 250))
        self.banner.delegate = self
        self.banner.location = ANLocation.getWithLatitude(kEplanningNewYorkLocationLatitudeFull, longitude: kEplanningNewYorkLocationLongitudeFull, timestamp: Date(), horizontalAccuracy: kEplanningNewYorkHorizontalAccuracy)
        stubRequestWithResponse("SuccessfulLocationCreativeForBannerAdResponse")
        expectationRequest = expectation(description: "\(#function)")
        expectationResponse = expectation(description: "\(#function)")
        banner.loadAd()
        ...
        if let deviceDic = jsonRequestBody["device"] as? [String : Any], let geoDic = deviceDic["geo"] as? [String : Any]
        {
            ...
            XCTAssertEqual(banner.location?.longitude, kEplanningNewYorkLocationLongitudeFull)
            XCTAssertEqual(banner.location?.horizontalAccuracy, kEplanningNewYorkHorizontalAccuracy)
    ...
    func test_TC7_ExternalUidInBanner() {
        self.banner = ANBannerAdView.init(frame: CGRect(x: 0, y: 0, width: 300, height: 250), memberId: memberID, inventoryCode: inventoryCode, adSize: CGSize(width: 300, height: 250))
        self.banner.delegate = self
        self.banner.externalUid = "Eplanning"
        stubRequestWithResponse("SuccessfulLocationCreativeForBannerAdResponse")
        expectationRequest = expectation(description: "\(#function)")
        expectationResponse = expectation(description: "\(#function)")
        banner.loadAd()
