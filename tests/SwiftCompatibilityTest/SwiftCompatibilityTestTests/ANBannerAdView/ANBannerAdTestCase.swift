...
class AANBannerAdTestCase: XCTestCase, ANBannerAdViewDelegate {
    ...
    func validateGenericNativeAdObject()
    {
        if nativeAd?.title != nil {
            XCTAssert(nativeAd?.title == "EplanningSDKApp")
        }
        if nativeAd?.body != nil {
            XCTAssert(nativeAd?.body == "Showcases the Eplanning mobile SDK and all its features. Works with placements generated under a publisher in the Eplanning Console.")
        }
        ...
        if nativeAd?.sponsoredBy != nil {
            XCTAssert(nativeAd?.sponsoredBy == "Eplanning Sponsored")
        }
