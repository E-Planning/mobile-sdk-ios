...
@interface EPLOMIDImplementation()
...
@implementation EPLOMIDImplementation

+ (instancetype)sharedInstance {
    static dispatch_once_t omidMicrosoftToken;
    static EPLOMIDImplementation *omidMicrosoftImplementation;
    dispatch_once(&omidMicrosoftToken, ^{
        omidMicrosoftImplementation = [[EPLOMIDImplementation alloc] init];
...
- (void) activateOMIDandCreatePartner
{
    ...
    if(!OMIDMicrosoftSDK.sharedInstance.isActive){
        ...
        self.partner = [[OMIDMicrosoftPartner alloc] initWithName: EPL_OMIDSDK_PARTNER_NAME
                                                   versionString: EPL_SDK_VERSION];
    }
...
-(OMIDMicrosoftAdSession*) initialseOMIDAdSessionForView:(id)view withSessionContext:(OMIDMicrosoftAdSessionContext*)context andImpressionOwner:(OMIDOwner)impressionOwner andMediaEventsOwner:(OMIDOwner)mediaEventsOwner htmlAd:(BOOL)isBannerAd
{
    ...
    @try {
        ...
        EPLLogError(@"Attempt to add view failed");
    }
    // Start session
    [omidAdSession start];
...
- (void) fetchOMIDJS
{
    ...
    NSString *omSdkJSPath = EPLPathForANResource(kANOMIDSDKJSFilename, @"js");
    if (!omSdkJSPath) {
        return;
    }
...
- (NSString *)getOMIDJS
{
    ...
    @synchronized (self) {
        ...
        if (!scriptContent) {
            EPLLogWarn(@"scriptContent is nil");
            scriptContent=  @"";
        }
    }
